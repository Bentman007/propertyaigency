    ...
    <main className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400">Each credit = one listing, live for 2 months. Use 1 credit to relist.</p>
          {profile?.bundle_credits > 0 && (
            <div className="mt-4 inline-block bg-gray-800 border border-orange-500 rounded-xl px-6 py-3">
          ...
          {BUNDLES.map(bundle => (
            <button key={bundle.key} onClick={() => setSelected(bundle.key)}
              className={`relative rounded-2xl p-5 border-2 text-left transition ${
                selected === bundle.key ? 'border-orange-500 bg-gray-800' : 'border-gray-700 bg-gray-800 hover:border-gray-500'
              }`}>
              {bundle.popular && (
              ...
              <p className="text-gray-400 text-xs mt-1">{bundle.credits} credits</p>
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 mb-6">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-400">{selectedBundle.name} Bundle</span>
              <span className="font-bold">R{selectedBundle.price.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Listing credits</span>
              <span className="text-green-400 font-bold">{selectedBundle.credits} credits</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Cost per listing</span>
              <span className="text-green-400 font-bold">R{selectedBundle.perListing}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Listing duration</span>
              <span>2 months each</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Lock-in contract</span>
        ...
        <div className="text-center">
          <div className="bg-gray-800 border border-yellow-600 rounded-2xl p-6 mb-4">
            <p className="text-yellow-400 font-bold text-lg mb-2">💳 Online Payment Coming Soon</p>
            <p className="text-gray-400 text-sm mb-4">We are setting up our payment system. In the meantime, contact us to activate your bundle manually.</p>
            <Link href="/contact"
          ...
          <p className="text-gray-500 text-xs">Once PayFast is live, payment will be instant and credits added automatically.</p>
        </div>
      </div>
    </main>
  )
