...
export default function AdminSuppliers() {
  ...
  const tierColour: Record<string, string> = {
    ...
    standard: 'text-green-400', trade: 'text-stone-700',
  }
    ...
    <main className="min-h-screen bg-stone-50 text-stone-900">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-stone-500 text-sm mt-1">Review, approve and manage service providers</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-stone-300 rounded-2xl p-12 text-center">
          ...
          <div className="space-y-4">
            {suppliers.map(sup => (
              <div key={sup.id} className="bg-white border border-stone-300 rounded-2xl overflow-hidden">

                <div className="px-6 py-4 border-b border-stone-300 flex items-start justify-between gap-4">
                  <div className="flex items-center gap-4">
                    {sup.logo_url
                      ? <img src={sup.logo_url} alt="" className="w-14 h-14 rounded-xl object-contain bg-stone-100 flex-shrink-0"/>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className={`text-xs font-semibold uppercase ${tierColour[sup.lead_tier] || 'text-stone-500'}`}>
                          {sup.lead_tier} · R{sup.lead_price}/lead
                        </span>
                        <span className="text-stone-400">·</span>
                        <span className="text-stone-500 text-xs capitalize">{sup.service_type?.replace(/_/g, ' ')}</span>
                      </div>
                      <p className="text-stone-400 text-xs mt-1">{sup.areas_served?.join(', ')}</p>
                    </div>
                  </div>
                ...
                <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div>
                      <p className="text-stone-400 text-xs uppercase tracking-wider mb-1">Contact</p>
                      <p className="text-sm">{sup.email}</p>
                      {sup.phone && <p className="text-sm text-stone-500">{sup.phone}</p>}
                    </div>
                    {sup.website && (
                      <div>
                        <p className="text-stone-400 text-xs uppercase tracking-wider mb-1">Website</p>
                    ...
                    <div>
                      <p className="text-stone-400 text-xs uppercase tracking-wider mb-1">Weekly Lead Limit</p>
                      <p className="text-sm">{sup.weekly_lead_limit} leads/week</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-stone-400 text-xs uppercase tracking-wider mb-1">AI Profile</p>
                    <p className="text-sm text-stone-700 leading-relaxed line-clamp-4">{sup.ai_profile || sup.description || 'No profile written'}</p>
                  </div>
                </div>

                {filter === 'pending_review' && (
                  <div className="px-6 py-4 border-t border-stone-300 space-y-3">
                ...
                {filter === 'rejected' && sup.rejection_reason && (
                  <div className="px-6 py-3 border-t border-stone-300 bg-red-950">
                    <p className="text-red-400 text-xs">Reason: {sup.rejection_reason}</p>
                  </div>
                )}

                {filter === 'active' && (
                  <div className="px-6 py-3 border-t border-stone-300 flex items-center justify-between text-sm">
                    <div className="flex items-center gap-4 text-stone-500">
                      <span>Leads: <span className="text-stone-900 font-bold">{sup.total_leads_received || 0}</span></span>
                      <span>Rating: <span className="text-stone-900 font-bold">{sup.rating || '—'}</span></span>
