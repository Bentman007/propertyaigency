    ...
    <main className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        <div className="mb-8">
          ...
          <p className="text-gray-400">Upload multiple properties at once using a CSV file</p>
        </div>
          ...
          {[1, 2, 3].map(s => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                step >= s ? 'bg-orange-500 text-black' : 'bg-gray-700 text-gray-400'
              }`}>{s}</div>
              <span className="text-sm text-gray-400">
                {s === 1 ? 'Upload CSV' : s === 2 ? 'Preview' : 'Done'}
              </span>
              {s < 3 && <div className={`w-12 h-0.5 ${step > s ? 'bg-orange-500' : 'bg-gray-700'}`}/>}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div className="bg-gray-800 border border-gray-700 rounded-2xl p-8 text-center">
              ...
              <p className="text-gray-400 text-sm mb-6">Your CSV must have headers matching our template</p>
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6">
              ...
              <div className="bg-gray-700 rounded-lg p-3 text-xs font-mono text-gray-300 overflow-x-auto mb-4">
              ...
              <div className="text-sm text-gray-400 space-y-1 mb-4">
                <p>• <strong className="text-white">price_type:</strong> "sale" or "rent"</p>
                <p>• <strong className="text-white">property_type:</strong> house, apartment, townhouse, farm, etc</p>
                <p>• <strong className="text-white">price:</strong> numbers only, no R symbol</p>
              </div>
                ...
                {csvData.map((row, i) => (
                  <div key={i} className="px-5 py-3 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-gray-400 text-xs">{row.suburb}, {row.city} · {row.bedrooms} beds · R{parseInt(row.price).toLocaleString()}</p>
                    </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className={`border rounded-xl p-4 text-center ${errorCount > 0 ? 'bg-red-900 border-red-700' : 'bg-gray-800 border-gray-700'}`}>
                <p className={`text-3xl font-bold ${errorCount > 0 ? 'text-red-300' : 'text-gray-400'}`}>{errorCount}</p>
                <p className={`text-sm ${errorCount > 0 ? 'text-red-400' : 'text-gray-500'}`}>Failed</p>
              </div>
            </div>
            ...
            {errorCount > 0 && (
              <div className="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
                ...
                {results.filter(r => r.status === 'error').map((r, i) => (
                  <div key={i} className="px-5 py-3 border-b border-gray-700">
