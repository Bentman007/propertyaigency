      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">My Listings</p>
          </div>
          <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 text-center">
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {pendingBookings.map(booking => (
                <div key={booking.id} className="bg-gray-800 border border-yellow-500 rounded-xl p-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => confirmBooking(booking.id, 'declined')}
                      className="flex-1 bg-gray-700 hover:bg-red-700 text-gray-300 hover:text-white py-2 rounded-lg text-sm transition">
                      Decline
                    </button>
                  </div>
                </div>
          ...
          {properties.length === 0 ? (
            ...
            <div className="space-y-4">
              {properties.map(property => (
                <div key={property.id} className="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ${
                          ...
                          property.status === 'draft' ? 'bg-gray-600 text-gray-300' :
                          'bg-yellow-900 text-yellow-300'
                        }`}>{property.status}</span>
                      </div>
                      <p className="text-gray-400 text-xs mt-0.5">📍 {property.suburb}, {property.city}</p>
                      <p className="text-orange-500 font-bold mt-1">{formatPrice(property.price, property.price_type)}</p>
                      <p className="text-gray-500 text-xs">🛏 {property.bedrooms} beds · 🚿 {property.bathrooms} baths</p>
                    </div>
                  </div>
                  ...
                  <div className="px-4 pb-4 flex flex-wrap gap-2">
                    <Link href={`/property/${property.id}`}
                      className="text-sm bg-gray-700 hover:bg-gray-600 text-gray-300 px-3 py-1.5 rounded-lg transition">
                      👁 View
                    </Link>
                    ...
                    {property.status === 'active' && (
                      <button onClick={() => updateStatus(property.id, 'sold')}
                        className="text-sm bg-gray-700 hover:bg-red-800 text-gray-300 px-3 py-1.5 rounded-lg transition">
                        ✅ Mark Sold/Rented
                      </button>
                    )}
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {confirmedBookings.map(booking => (
                <div key={booking.id} className="bg-gray-800 border border-green-700 rounded-xl p-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-sm mb-4">Set the days and times buyers can book viewings of your property</p>
                ...
                <div key={day} className="flex items-center gap-3">
                  <button onClick={() => toggleDay(day)}
                    className={`w-28 text-sm font-semibold py-2 rounded-lg transition flex-shrink-0 ${
                      availability[day] ? 'bg-orange-500 text-black' : 'bg-gray-700 text-gray-400'
                    }`}>
                    {day.slice(0, 3)}
                  </button>
                  {availability[day] ? (
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-gray-400 text-sm mb-4">Please let us know why you are removing this listing</p>
