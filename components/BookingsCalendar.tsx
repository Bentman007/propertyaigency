...
export default function BookingsCalendar({ agentId }: { agentId: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-300 p-6">
          ...
          return (
            <button key={day}
              ...
              className={`relative rounded-lg p-1 min-h-10 text-center transition ${
                ...
                isToday ? 'bg-stone-200 text-stone-900' :
                dayBookings.length > 0 ? 'bg-stone-100 hover:bg-stone-200 text-stone-900' :
      ...
      {selectedDay && (
        <div className="border-t border-stone-300 pt-4">
          <p className="font-semibold mb-3">
            ...
            <span className="text-stone-500 text-sm ml-2">({selectedBookings.length} booking{selectedBookings.length !== 1 ? 's' : ''})</span>
          </p>

          {selectedBookings.length === 0 ? (
            <p className="text-stone-400 text-sm">No bookings on this day</p>
          ) : (
                ...
                <div key={booking.id} className="bg-stone-100 rounded-xl p-4">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-stone-500 text-xs">{booking.property_address || `${booking.properties?.suburb}, ${booking.properties?.city}`}</p>
                      <p className="text-orange-500 font-bold text-sm mt-1">⏰ {booking.start_time}</p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full text-stone-900 font-semibold ${getStatusColor(booking.status)}`}>
                      {booking.status}
                    </span>
                  </div>

                  {booking.searcher_profile && Object.keys(booking.searcher_profile).length > 0 && (
                    <div className="bg-stone-200 rounded-lg p-2 mb-3">
                      <p className="text-xs text-stone-700 font-semibold mb-1">🤖 Buyer Profile:</p>
                      <div className="flex flex-wrap gap-2">
                        {booking.searcher_profile.budget_max && (
                          <span className="text-xs bg-gray-500 text-stone-800 px-2 py-0.5 rounded">💰 R{booking.searcher_profile.budget_max?.toLocaleString()}/mo</span>
                        )}
                        {booking.searcher_profile.move_timeline && (
                          <span className="text-xs bg-gray-500 text-stone-800 px-2 py-0.5 rounded">📅 {booking.searcher_profile.move_timeline}</span>
                        )}
                        {booking.searcher_profile.has_kids && (
                          <span className="text-xs bg-gray-500 text-stone-800 px-2 py-0.5 rounded">👨‍👩‍👧 Kids</span>
                        )}
                        {booking.searcher_profile.has_pets && (
                          <span className="text-xs bg-gray-500 text-stone-800 px-2 py-0.5 rounded">🐾 Pets</span>
                        )}
                  ...
                  {booking.status === 'pending' && (
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => handleAction(booking.id, 'decline')}
                        disabled={processing === booking.id}
                        className="flex-1 bg-red-800 hover:bg-red-700 text-stone-900 font-bold py-2 rounded-lg text-sm disabled:opacity-50">
                        ❌ Decline
                      </button>
                    </div>
                  )}
      ...
      {/* No bookings at all */}
      {bookings.length === 0 && (
        <p className="text-stone-400 text-sm text-center py-4">No viewings booked yet</p>
      )}
    </div>
  )
}
