      ...
      <div className="bg-amber-50 rounded-xl p-4 border border-orange-500">
        <div className="flex gap-3">
          ...
          <div>
            ...
            <button className="ml-3 text-stone-400 hover:text-stone-700 text-xs">No thanks</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-amber-50 rounded-xl p-4 mb-3 space-y-3">
      ...
      {isRent ? (
        <>
          <div>
            <label className="text-xs text-stone-500 mb-1 block">Desired move-in date</label>
            <input type="date" value={form.move_date}
              onChange={e => setForm(p => ({ ...p, move_date: e.target.value }))}
              min={new Date(Date.now() + 86400000).toISOString().split('T')[0]}
              className="w-full bg-stone-200 text-stone-900 rounded-lg px-3 py-2 text-sm outline-none border border-gray-500 focus:border-orange-500"/>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs text-stone-500 mb-1 block">Lease length</label>
            ...
            <div>
              <label className="text-xs text-stone-500 mb-1 block">Occupants</label>
        ...
        <>
          <div>
            <label className="text-xs text-stone-500 mb-1 block">Offer amount (R)</label>
          ...
          <div>
            <label className="text-xs text-stone-500 mb-1 block">Finance type</label>
          ...
          <div>
            <label className="text-xs text-stone-500 mb-1 block">Desired occupation date</label>
            <input type="date" value={form.move_date}
              onChange={e => setForm(p => ({ ...p, move_date: e.target.value }))}
              className="w-full bg-stone-200 text-stone-900 rounded-lg px-3 py-2 text-sm outline-none border border-gray-500 focus:border-orange-500"/>
          </div>
      ...
      <div>
        <label className="text-xs text-stone-500 mb-1 block">Message to agent (optional)</label>
      ...
      <p className="text-xs text-stone-400 text-center">The agent will respond via your dashboard</p>
    </div>
  )
}
